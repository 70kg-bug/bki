...
recomputed.
    ...
    python -m pipeline.tools.gate_pivot              # run the whole sequence
    python -m pipeline.tools.gate_pivot --score S3   # score the current build only
"""
...
from ..stages.s11_train import load
from ..stages.s12_baselines import bootstrap_metric_diff
from ..stages.s13_calibrate import fit_platt, fit_xgb, score, tuned_params, xgb_predict

REPORT_JSON = C.RPT_TOOL_GATE_PIVOT
...
def _run_step(tag: str, env: dict[str, str]) -> dict:
    ...
    for args, label in (
            ...
            (["-m", "pipeline.tools.gate_pivot", "--score", tag], "score")):
